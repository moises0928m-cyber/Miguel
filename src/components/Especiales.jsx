...
export default function Especiales() {
  ...
  const [imagenes, setImagenes] = useState([]); // Archivos reales
  ...
  // ✅ Función corregida para subir a Cloudinary
  const cloudinaryUpload = async (file) => {
    try {
      const data = new FormData();
      data.append("file", file);
      data.append("upload_preset", "Nogales"); // ⬅️ Cambiado a ml_default
  ...
  // ✅ Función corregida para enviar a WhatsApp
  const Whasap = async (e) => {
    ...
    if (imagenes.length > 0) {
      ...
      for (let i = 0; i < imagenes.length; i++) {
        try {
          console.log(`⏳ Subiendo imagen ${i + 1}/${imagenes.length}...`);
          ...
          if (url) {
            ...
            console.log(`❌ Error subiendo imagen ${i + 1}`);
          }
        } catch (error) {
    ...
    console.log("🎯 URLs finales obtenidas:", urls);
...
${
  urls.length > 0
    ...
    console.log("📨 Mensaje final para WhatsApp:", mensaje);
